...
Hint: you can use the `plot` function, `pairs` function or `ggpairs` function.
...
resid_vs_complaints <- ggplot(data = super) +
  geom_point(mapping = aes(x = Complaints, y = linear_model$residuals)) +
...
plot(resid_vs_complaints)
plot(resid_vs_privileges)
plot(resid_vs_learn)
plot(resid_vs_raises
plot(resid_vs_critical)
plot(resid_vs_advance)
```
...
```{r, fig.align='center'}
# your code here
# (create a Histogram, QQ-plot, and shapiro wilk test)
```
...
# added the residuals, fitted values, etc.)
```
...
Hint: use the `vif` function from the car library with your fitted model as the argument to the function.
...
#                          Advance, 
#                        data = super)
# anova(your_full_lm_object, super_lm_reduced) # Must put it your own lm from previously
```
